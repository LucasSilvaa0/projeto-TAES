...
def analisar_com_chat(codigo_swift, data_start):
    try:
        new_data = data_start
        new_data["prompt"] = f"Analise este código:\n\n```swift\n{codigo_swift}\n```"
        
        response = requests.post(URL, json=new_data, timeout=90)
        texto_limpo = response.text.replace("```json", "").replace("```", "").strip()
        return json.loads(texto_limpo)
    except Exception as e:
        return {"is_flaky": False, "error": True, "error_msg": str(e)}

def enviar_feedback(acertou, era_flaky, motivo_modelo):
    ...
    try:
        requests.post(URL, json=msg, timeout=90)
    except Exception as e:
...
def main():
    ...
    try:
        requests.post(URL, json=data_start, timeout=90)
    except:
        print("ERROR")
        return
    ...
    for i, (caminho, is_realmente_flaky) in enumerate(dataset):
        ...
        try:
            ...
            # Chama a LLM
            predicao = analisar_com_chat(codigo, data_start)
            ...
            # Feedback
            enviar_feedback(acertou, is_realmente_flaky, reason)
