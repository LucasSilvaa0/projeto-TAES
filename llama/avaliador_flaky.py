...
def enviar_feedback(chat, acertou, era_flaky, motivo_modelo):
...
def main():
    ...
    os.makedirs(FLAKY_DIR)
    descompactar_zip("flaky.zip", FLAKY_DIR)

    if os.path.exists(NON_FLAKY_DIR):
        shutil.rmtree(NON_FLAKY_DIR)
